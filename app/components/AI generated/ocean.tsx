...
void main() {
    ...
    // Add perspective (squash the Y axis to make it look like we are looking across the ocean)
    p.y *= 1.5; 

    float time = uTime * uSpeed;
    
    // Wave height accumulator
    float h = 0.0;
    float amp = 1.0;
    float freq = uFrequency;
    vec2 current_p = p;

    // Generate long, rolling domain-warped waves
    for (int i = 0; i < 4; i++) {
        // Bend the horizontal waves organically using the X axis
        ...
        // Accumulate height (primarily horizontal lines)
        h += sin(current_p.y * freq + time * 1.2) * amp;
        
        freq *= 1.8; // Increase frequency for finer details
        amp *= 0.5;  // Decrease amplitude for those details
    }

    // Normalize height mapping roughly to [-1.0, 1.0]
    h /= 1.8;
    ...
    // Add seafoam highlights only to the highest peaks
    col = mix(col, foamColor, smoothstep(0.5, 1.0, h) * uFoam);

    // Subsurface scattering / Light reflection on the waves
...
const DEFAULT_PARAMS: Required<OceanProps> = {
  speed: 0.5,
  scale: 3.0,
  distortion: 1.2,
  frequency: 1.5,
