...
void main() {
    ...
    // Mouse Displacement: Gentle fluid push
    ...
    // Generate organic, sweeping tidal lines
    for(int i = 0; i < 3; i++) {
        // Lateral warping (bends the horizontal lines into organic tide shapes)
        float xWarp = sin(p.x * freq * 0.8 + time * 0.5) * uDistortion;
        
        // The main directional pull
    ...
    // Multiply by complexity to create distinct tidal "bands" or terraces
    float bands = wave * uComplexity;
    
    // fract() creates the repeating ramp (water building up to a crest)
    float crest = fract(bands);
    
    // Foam logic: concentrated right at the peak of the crest before it drops
    float foamEdge = smoothstep(0.8 - (uFoam * 0.1), 0.98, crest);
    
    // Add micro-noise to the foam so the leading edge breaks organically
    ...
    // Ocean Tide Color Palette
    vec3 deepWater = vec3(0.01, 0.05, 0.15) + uColorShift * 0.1; // Dark trench
    vec3 shallowWater = vec3(0.05, 0.45, 0.6) + uColorShift * 0.2; // Aqua shallows
    vec3 foamColor = vec3(0.85, 0.95, 1.0); // White/cyan froth

    // Gradient from deep to shallow as the tide rolls forward
    vec3 col = mix(deepWater, shallowWater, crest);
    
    // Add the foam cap at the leading edge
    col = mix(col, foamColor, finalFoam);

    // Subtle subsurface glow under the crest
...
const DEFAULT_PARAMS: Required<TidesProps> = {
  speed: 0.3,
  scale: 3.0,
  distortion: 1.5,
  frequency: 1.0,
