...
void main() {
    ...
    // Mouse Interaction: Acts as a gentle physical breeze pushing the flower head
    ...
    // Natural leafy background bokeh
    vec3 bg = vec3(0.02, 0.05, 0.02); // Deep forest green
    bg += noise(p * 3.0) * 0.02; // Subtle leaf texture

    vec3 finalCol = bg;

    // Natural Rose/Peony Color Palette
    vec3 coreCol = vec3(0.15, 0.0, 0.02); // Deep crimson core
    vec3 midCol = vec3(0.85, 0.15, 0.3);  // Rich petal red/pink
    vec3 edgeCol = vec3(0.95, 0.75, 0.7); // Creamy peach edges

    // Render 15 overlapping layers of petals (Back to Front)
    for(int j = 0; j < 15; j++) {
        float i = float(j);
        float t = i / 14.0; // 0.0 (bottom layer) to 1.0 (top layer)
        
        // The radius size of this specific layer of petals
        float layerRadius = (0.2 + t * 0.8) * uBloom;
        
        // Number of petals varies naturally per layer
        float numPetals = floor(uPetals + t * 3.0);
        
        // Stagger rotation using the Golden Ratio to simulate natural plant growth (Phyllotaxis)
        float rot = i * 2.39996 + sin(wind * 0.5 + i) * 0.05;
        float localA = a + rot;
        
        // Separate the circle into distinct petals
        float petalID = floor(localA * numPetals / 6.28318);
        float angleFract = fract(localA * numPetals / 6.28318) * 2.0 - 1.0;
        
        // Shape the petal (Wider at base, rounded at top)
        float shape = 1.0 - pow(abs(angleFract), 1.8 + t);
        
        // Add micro-noise to the petal edge so it ruffles
        float edgeNoise = noise(vec2(petalID, r * 10.0)) * 0.1;
        shape += edgeNoise;

        // Calculate if we are inside the petal boundary
        float boundary = r / max(0.001, (layerRadius * shape));
        
        // 1. Cast a drop shadow onto the layers BELOW this one
        float shadow = smoothstep(1.15, 0.95, boundary);
        finalCol = mix(finalCol * 0.6, finalCol, shadow);

        // 2. Draw the actual petal
        float mask = smoothstep(1.0, 0.98, boundary);
        if (mask > 0.0) {
            // Biological color gradient from base to tip
            vec3 col = mix(coreCol, midCol, smoothstep(0.0, 0.5, boundary));
            col = mix(col, edgeCol, smoothstep(0.4, 1.0, boundary));
            
            // Subtle color variance per layer so they don't blend together perfectly
            col *= 0.9 + 0.1 * noise(vec2(i * 10.0, 0.0));
            
            // User-defined Hue Shift
            ...
            // Velvet SSS (Subsurface Scattering) - Petal edges glow as light passes through
            float velvet = smoothstep(0.8, 1.0, boundary);
            col += edgeCol * velvet * uVelvet;

            // Petal Curl (An inner shadow right before the bright edge to simulate 3D thickness)
            float curl = smoothstep(0.7, 0.9, boundary) * smoothstep(1.0, 0.9, boundary);
            col *= 1.0 - curl * 0.4;

            // Ambient Occlusion (Darken the deep center of the flower)
    ...
    // Soft Biological Center (Stamen/Pollen Core)
    float coreMask = smoothstep(0.15 * uBloom, 0.0, r);
    vec3 stamenCol = vec3(0.9, 0.6, 0.1); // Rich pollen yellow
    stamenCol *= 0.5 + 0.5 * noise(p * 50.0); // Bumpy texture
    finalCol = mix(finalCol, stamenCol, coreMask * 0.9);

    // Global Vignette (Mimics macro photography lens falloff)
...
const DEFAULT_PARAMS: Required<OrganicFlowerProps> = {
  speed: 0.5,
  scale: 2.0,
  bloom: 1.2,
  petals: 4.0,
...
export function OrganicFlowerCanvas(props: OrganicFlowerProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full relative overflow-hidden block bg-[#030603]"
    >
