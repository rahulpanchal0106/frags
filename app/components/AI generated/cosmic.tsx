...
void main() {
    ...
    // 2. Normalize mouse to match UV space
    vec2 mousePos = (uMouse - 0.5) * (uResolution.xy / min(uResolution.y, uResolution.x)) * uScale;
    
    // Original un-warped position
    vec2 p = uv;

    // 3. Apply Mouse Liquid Drag / Ripple
    vec2 mDir = p - mousePos;
    float d = length(mDir);
    // Radius of influence (the lower the multiplier, the bigger the splash)
    float pushEffect = exp(-d * 4.0); 
    
    // Drag the water in the direction of the velocity
    uv -= uMouseVelocity * pushEffect * uMouseForce * 2.0;
    
    // Add a slight ripple burst effect
    uv += normalize(mDir) * sin(d * 15.0 - uTime * 10.0) * pushEffect * (length(uMouseVelocity) * uMouseForce) * 0.1;

    // 4. Domain warping / Fluid mechanics
    ...
    // 5. Color mapping based on warped space
    ...
    // Apply brightness and center glow
    col *= uBrightness;
    
    // Add a dynamic glow near the mouse 
...
// Fallback values
const DEFAULT_PARAMS: Required<CosmicFluidProps> = {
  speed: 1.0,
  scale: 2.0,
  colorShift: 0.0,
  distortion: 3.0,
...
export function CosmicFluidCanvas(props: CosmicFluidProps) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Store normalized coordinates [0, 1]. Invert Y because WebGL Y goes up.
    ...
    const loop = () => {
      ...
      // 1. Calculate physics for smooth mouse
      m.x += (m.targetX - m.x) * 0.1;
      m.y += (m.targetY - m.y) * 0.1;

      // 2. Calculate velocity (current pos - last pos)
      m.vx = (m.x - m.lastX) * 15.0; // scale up slightly for the shader
      ...
      // 4. Pass everything to the shader
