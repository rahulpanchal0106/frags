...
// Random 2D Hash for generating cell points
vec2 hash2(vec2 p) {
...
void main() {
    ...
    // Mouse Interaction: Sharp geometric shift
    ...
    // Scale and grid complexity
    vec2 p = uv * uScale * uComplexity;
    
    vec2 g = floor(p); // Grid cell
    vec2 f = fract(p); // Local cell coordinates
    
    vec2 best_p;
    vec2 best_n;
    float best_d = 8.0;
    
    // Pass 1: Find the nearest animating point
    for(int y = -1; y <= 1; y++) {
        for(int x = -1; x <= 1; x++) {
            vec2 n = vec2(float(x), float(y));
            vec2 offset = hash2(g + n);
            
            // Animate the points randomly
    ...
    // Pass 2: Calculate precise mathematical distance to the sharp boundary edge
    float minDist = 8.0;
    for(int y = -2; y <= 2; y++) {
        for(int x = -2; x <= 2; x++) {
            vec2 n = vec2(float(x), float(y));
...
const DEFAULT_PARAMS: Required<ShardsProps> = {
  speed: 1.0,
  scale: 1.5,
  complexity: 4.0,
  colorShift: 0.1,
