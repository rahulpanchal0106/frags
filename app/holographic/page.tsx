...
const holoControls: ControlDef<HolographicProps>[] = [
  { key: "speed", label: "Flow Speed", min: 0.0, max: 1.0, step: 0.01 },
  { key: "scale", label: "Fabric Scale", min: 0.5, max: 4.0, step: 0.1 },
  {
    key: "complexity",
    label: "Fold Complexity",
    min: 0.5,
    max: 5.0,
    step: 0.1,
  },
  { key: "distortion", label: "Twist & Warp", min: 0.0, max: 3.0, step: 0.1 },
  {
    key: "iridescence",
    label: "Iridescence Spread (Rainbow)",
    min: 0.0,
    max: 4.0,
    step: 0.1,
  },
  { key: "darkness", label: "Obsidian Depth", min: 0.0, max: 1.0, step: 0.05 },
  {
    key: "mouseForce",
...
const generateHoloTemplate = (paramsJson: string) => `
"use client";
...
void main() {
    ...
    vec3 finalColor = baseColor * (1.0 - uDarkness) + (diffuse * 0.05); 
    finalColor += holoColor * spec * 2.5; 
    finalColor += holoColor * fresnel * 0.4; 
...
const DEFAULT_PARAMS: Required<HolographicProps> = ${paramsJson};
...
export default function HolographicWorkspace() {
  return (
    <ShaderController<HolographicProps>
      title="Obsidian Silk"
      initialValues={initialValues}
      controls={holoControls}
      renderShader={(params) => <HolographicCanvas {...params} />}
      exportTemplate={generateHoloTemplate}
    />
  );
}
